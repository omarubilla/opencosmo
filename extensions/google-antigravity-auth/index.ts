...
const CLIENT_ID = process.env.OPENCLAW_GOOGLE_ANTIGRAVITY_CLIENT_ID?.trim() ?? "";
const CLIENT_SECRET = process.env.OPENCLAW_GOOGLE_ANTIGRAVITY_CLIENT_SECRET?.trim() ?? "";
...
function requireGoogleOAuthClientConfig(): { clientId: string; clientSecret: string } {
  if (!CLIENT_ID || !CLIENT_SECRET) {
    throw new Error(
      "Missing Google OAuth client credentials. Set OPENCLAW_GOOGLE_ANTIGRAVITY_CLIENT_ID and OPENCLAW_GOOGLE_ANTIGRAVITY_CLIENT_SECRET.",
    );
  }
...
function buildAuthUrl(params: { challenge: string; state: string }): string {
  const { clientId } = requireGoogleOAuthClientConfig();
...
async function exchangeCode(params: {
  ...
  const { clientId, clientSecret } = requireGoogleOAuthClientConfig();
