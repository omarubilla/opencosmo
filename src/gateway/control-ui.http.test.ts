...
describe("handleControlUiHttpRequest", () => {
  async function withControlUiRoot<T>(params: {
    ...
    try {
      ...
      await fs.writeFile(path.join(tmp, "landing.html"), params.landingHtml ?? "<html>landing</html>\n");
  ...
  function readEndBody(end: ReturnType<typeof makeMockHttpResponse>["end"]): string {
  ...
  it("does not inject inline scripts into index.html", async () => {
    ...
    await withControlUiRoot({
      ...
      fn: async (tmp) => {
        ...
        const handled = handleControlUiHttpRequest(
          { url: "/chat", method: "GET" } as IncomingMessage,
          res,
          {
            root: { kind: "resolved", path: tmp },
            config: {
        ...
        expect(handled).toBe(true);
        expect(readEndBody(end)).toBe(html);
      },
    });
  });
  ...
  it("serves landing page at root", async () => {
    await withControlUiRoot({
      indexHtml: "<html><body>chat-app</body></html>\n",
      landingHtml: "<html><body>landing-page</body></html>\n",
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = handleControlUiHttpRequest(
          { url: "/", method: "GET" } as IncomingMessage,
        ...
        expect(handled).toBe(true);
        expect(readEndBody(end)).toBe("<html><body>landing-page</body></html>\n");
      },
    });
  });

  it("serves chat app for /chat via SPA fallback", async () => {
    await withControlUiRoot({
      indexHtml: "<html><body>chat-app</body></html>\n",
      landingHtml: "<html><body>landing-page</body></html>\n",
      fn: async (tmp) => {
        ...
        expect(handled).toBe(true);
        expect(end).toHaveBeenCalledWith("<html><body>chat-app</body></html>\n");
      },
    });
  });
});
